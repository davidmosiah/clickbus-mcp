...
export async function buildConnectionStatus(options: { env?: Env; homeDir?: string } = {}): Promise<ConnectionStatus> {
  ...
  if (!exists) missing.push("CLICKBUS_ACCESS_TOKEN");
  const next: string[] = [];
  if (!exists) {
    next.push("ClickBus has no consumer OAuth. Open the ClickBus app or a captured request to api.clickbus.com.");
    next.push("DevTools / mitm → copy Authorization, then clickbus-mcp-unofficial auth --from-header \"Bearer …\".");
  }
  if (!config.allowMutations) {
    next.push("Reads only. Book/cancel stay blocked until CLICKBUS_ALLOW_MUTATIONS is enabled AND explicit_user_intent.");
  }
  if (guest) next.push("Guest token cannot book or charge a ticket. Replace with a personal token.");
  ...
  return {
    ...
    next_steps: next,
    never_pays_by_default: true,
    auth_methods: ["auth --from-header", "auth --token", "CLICKBUS_ACCESS_TOKEN"]
  };
}
