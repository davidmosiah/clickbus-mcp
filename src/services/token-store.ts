...
import type { ClickbusTokenSet } from "../types.js";
...
export class TokenStore {
  ...
  async read(): Promise<ClickbusTokenSet | null> {
    try {
      const text = await fs.readFile(this.tokenPath, "utf8");
      return JSON.parse(text) as ClickbusTokenSet;
    } catch (error) {
  ...
  async write(tokens: ClickbusTokenSet): Promise<void> {
