...
import { TOOL_CALLS } from "../tools/clickbus-tools.js";
...
async function runSetup(args: string[]): Promise<number> {
  ...
  console.log("Next: clickbus-mcp-unofficial auth --from-header \"Bearer …\"");
  return 0;
}
...
async function runAuth(args: string[]): Promise<number> {
  ...
  const raw =
    headerIdx >= 0 ? args[headerIdx + 1] : idx >= 0 ? args[idx + 1] : process.env.CLICKBUS_ACCESS_TOKEN;
  const token = normalizeAccessToken(raw);
  if (!token || token.startsWith("--")) {
    console.error(`ClickBus has no consumer OAuth. Capture a personal ticket:

  1. Open the ClickBus app (or a captured request to api.clickbus.com)
  2. Copy Authorization then:
     clickbus-mcp-unofficial auth --from-header "Bearer eyJ…"
     clickbus-mcp-unofficial auth --token <ticket>

Place/trip search still work without a token; book stays blocked.
...
async function runDoctor(args: string[]): Promise<number> {
  ...
  if (args.includes("--json")) {
    ...
    console.log(`ClickBus MCP · Doctor  ${status.ok ? "READY" : "NEEDS AUTH"}`);
...
async function runCall(args: string[]): Promise<number> {
  ...
  if (!name || name.startsWith("-")) {
    console.error("Usage: clickbus-mcp-unofficial call <tool> [--json '{...}']");
...
function printHelp(): void {
  console.log(`clickbus-mcp-unofficial ${SERVER_VERSION}
Unofficial local-first ClickBus MCP. Never pays unless CLICKBUS_ALLOW_MUTATIONS and explicit_user_intent.

Commands:
  setup [--allow-mutations]   write ~/.clickbus-mcp/config.json (0600)
