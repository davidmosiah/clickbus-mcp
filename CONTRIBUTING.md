...
1. Keep the default read-only. Money tools stay behind `CLICKBUS_ALLOW_MUTATIONS` **and** `explicit_user_intent`.
2. Tests must import shipped handlers from `dist/services/handlers.js`, not a reimplementation.
3. Do not add live ClickBus credentials to fixtures.
