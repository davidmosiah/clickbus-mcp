...
const home = mkdtempSync(join(tmpdir(), "clickbus-cli-"));

function run(args, extraEnv = {}) {
  return new Promise((resolve, reject) => {
    const child = spawn(process.execPath, [bin, ...args], {
      env: { ...process.env, HOME: home, CLICKBUS_ACCESS_TOKEN: "", CLICKBUS_ALLOW_MUTATIONS: "", ...extraEnv },
...
const auth = await run(["auth", "--from-header", "Bearer clickbus-fixture-jwt"]);
...
assert.equal(stored.access_token, "clickbus-fixture-jwt");
...
assert.doesNotMatch(JSON.stringify(status), /CLICKBUS_ALLOW_MUTATIONS\s*=\s*true/);
...
const caps = await run(["call", "clickbus_capabilities", "--json", "{}"]);
...
const unknown = await run(["call", "clickbus_not_a_tool"]);
