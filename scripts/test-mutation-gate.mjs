import assert from "node:assert/strict";
import {
  MutationGateError,
  assertBookAllowed,
  assertCancelBookingAllowed,
  assertChargeWriteAllowed,
  assertLogoutAllowed,
  assertNotGuestForCharge
} from "../dist/services/mutation-gate.js";

function throws(fn, re) {
  try {
    fn();
    assert.fail("expected MutationGateError");
  } catch (error) {
    assert.equal(error instanceof MutationGateError, true);
    assert.match(error.message, re);
  }
}

throws(() => assertBookAllowed({ allowMutations: false, explicitUserIntent: true }), /CLICKBUS_ALLOW_MUTATIONS/);
throws(() => assertBookAllowed({ allowMutations: true, explicitUserIntent: false }), /explicit_user_intent/);
throws(() => assertBookAllowed({ allowMutations: true }), /explicit_user_intent/);
assertBookAllowed({ allowMutations: true, explicitUserIntent: true });

throws(() => assertCancelBookingAllowed({ allowMutations: false, explicitUserIntent: true }), /CLICKBUS_ALLOW_MUTATIONS/);
throws(() => assertChargeWriteAllowed({ allowMutations: true, explicitUserIntent: false }), /explicit_user_intent/);
throws(() => assertLogoutAllowed(false), /explicit_user_intent/);
assertLogoutAllowed(true);
throws(() => assertNotGuestForCharge("guest"), /guest token cannot book/i);

console.log(JSON.stringify({ ok: true, suite: "mutation-gate" }, null, 2));
