...
Same package, two doors. MCP registers tools on stdio/HTTP. The [skill](skill/SKILL.md) is the workflow (places → trips → stop before book) and can drive the **same** tools through the CLI when the client has no MCP:
